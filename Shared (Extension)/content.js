...
function isYouTube() {
  return /youtube\.com|youtu\.be/.test(window.location.href);
}

async function fetchCobalt(url) {
  const instances = [
    "https://cobalt.api.brecorder.com",
    "https://co.wuk.sh",
    "https://cobalt.synth.lol",
  ];
  ...
  return { url: null, error: "all instances failed" };
}

window.addEventListener("keydown", async (e) => {
  console.log("window keydown:", e.key, "alt:", e.altKey, "meta:", e.metaKey);
  ...
  if (isYouTube()) {
    const result = await fetchCobalt(window.location.href);
    console.log("cobalt result:", JSON.stringify(result));
    if (result?.url) {
      updatePopupUrl(result.url);
    } else {
      updatePopupError("Couldn't fetch: " + (result?.error || "unknown"));
    }
    return;
  }
